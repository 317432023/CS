    ...
    if (to.path === '/login') {
      ...
      if (hasRoles) {
        ...
        try {
          ...
          const { roleIds, roles } = await store.dispatch('security/getInfo')

          console.log('取得角色列表：\n' + JSON.stringify(store.state.security.roles)) // 这时候 store.state.security.roles 有值了

          // 基于角色信息生成可访问的动态路由表
          const accessRoutes = await store.dispatch('permission/generateRoutes', []/*roleIds 当使用空数组时后端会提取当前登录用户所有角色的路由*/)
