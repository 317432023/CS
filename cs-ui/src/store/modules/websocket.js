...
const actions = {
  CONNECT({ commit, state, dispatch }, params) {
    ...
    const { okCallBack, failCallBack } = params
    return new Promise((resolve, reject) => {
      state.stompClient.connect(headers, (frame) => {
        ...
        if ((typeof error) === 'object') {
          const errDetails = JSON.stringify(error)
          if (failCallBack) {
            failCallBack()
          }
        ...
        if (state.reConnectCount >= 5) {
          ...
          const tipMsg = '温馨提示：您的连接已断开，请退出后重新进入。'
  ...
  WEBSOCKET_UNSUBSCRIBE({ commit, state }, address) {
    return new Promise((resolve, reject) => {
      ...
      if (address) {
        let id = ''
        for (const item in state.stompClient.subscriptions) {
      ...
      // 取消所有订阅
      for (const id in state.stompClient.subscriptions) {
        state.stompClient.unsubscribe(id)
